...
# def generate_inline_keyboard(buttons_list):
#     keyboard = []
...
def ask_for_category_name(update, context):
    ...
    logger.info("Category Searched")
    if name_of_all_categories:
    #     inline_keyboard = tamplate_for_inline_keyboard([
    #         KeyboardButton(text="Show categories", callback_data="show_categories")])
...
show_categories = ConversationHandler(
    entry_points=[show_categories_command, CommandHandler('end', cancel_show_categories)],
    states={
        ASK_FOR_CATEGORY_NAME: [
            MessageHandler(
                Filters.text, 
        ...
        SHOW_LIST_OF_PRODUCTS: [
            ...
            CallbackQueryHandler(
                catch_previus, 
                pattern=pattern_identifier + PATTERN_TO_CATCH_THE_PREVIUS_PRODUCT
            ),
            CallbackQueryHandler(
                catch_details,
    ...
    fallbacks=[CommandHandler('cancel', cancel_show_categories), CommandHandler('quit', cancel_show_categories)]
)
